...
> **実装の現状（この要件定義からの差分・2024/2025 更新）**
>
> 本書はプロジェクト初期の要件定義です。実装は進み、以下が現状の挙動です。
> 最新のキー操作・使い方は [README.md](README.md) を参照してください。
>
> - **形態**: CLI検証に加え、**全アプリ対応の常駐（`conversion-service` + `hook-dll`）**を実装。
>   WH_KEYBOARD_LL + SendInput でどのアプリでもライブ変換が効く。**単一インスタンス**保証。
> - **誤字補正（もしかして）**: 当初検討したローカルLLM校正は **CPUでの費用対効果が低く廃止**。
>   代わりに「**自動変換に失敗した時だけ**」補正を出す2段構え（LLM不要・即時）に変更。
>   ①ローマ字取り残し（英字が残って失敗）を辞書＋ローマ字編集で補正、②かなになったが実在語に
>   変換できていない（未知語/カタカナ化）ケースを辞書のあいまい検索＋変換コストで実在語に寄せる
>   （例 きづついて→傷ついて）。正しく変換できている入力には出さない。
> - **候補一覧のページング**: 同音異義語は1ページ9件で固定し、9件超は自動ページ切替＋位置表示。
>   縦に伸びない。予測一覧も同じ描画で統一。
> - **誤学習のリセット（Delete）**: 候補一覧／予測一覧で選択中の変換の学習を Delete で1件だけ取消。
> - **入力モードは環境（ウィンドウ）ごと**に独立して記憶（ターミナルAは日本語、Bはコマンド等）。
> - **コマンドモード**: ターミナル（Windows Terminal / PowerShell / cmd / Git Bash / VSCode統合
>   ターミナル）でよく使うコマンドを学習・提案。Tab/Shift+Tabで一覧選択、Enterで実行、
>   **Delete で選択中のコマンド履歴を削除**。
> - **設定はモードで出し分け（`Ctrl+Alt+A`）**: 日本語変換モード → **単語登録**ウィンドウ（辞書に無い
>   複合語を「読み→表記」で登録、ライブ辞書へ即注入）／コマンドモード → **コマンド/エイリアス設定**
>   ウィンドウ（登録・編集・削除・検索・並び替え、スクリプト選択、`auto_run` フラグ）。
> - **キー割当の変更**: ←→ はカーソル移動（2回連打で行端）。Shift+Space（旧LLM校正）は廃止。
>
> ---
